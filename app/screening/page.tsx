            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 mt-1">
                  {t("stepWord")} {currentStepIndex + 1} {t("ofWord")}{" "}
                  {steps.length}: {steps[currentStepIndex].label}
                </p>
              </div>
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      ...
                      const filtered = array.filter((f) => {
                        if (f.size > maxSize) {
                          ...
                          console.warn(
                            `${f.name} exceeds 10MB and will be skipped.`
                          );
                          return false;
                        }
                        return true;
                  ...
                  {previews.length > 0 || files.length > 0 ? (
                    <div className="mt-4 grid grid-cols-3 gap-3">
                      {files.map((f, idx) => (
                        <div
                          key={idx}
                          className="border rounded p-2 bg-slate-50 dark:bg-slate-900"
                        >
